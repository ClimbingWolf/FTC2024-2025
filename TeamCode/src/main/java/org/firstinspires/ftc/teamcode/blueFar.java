...
public class blueFar extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        claw.setPosition(posB);
        brr = readFromFile(fileStart + "brr.txt");
        brm = readFromFile(fileStart +"brm.txt");
        brl = readFromFile(fileStart +"brl.txt");
        ...
        while (opModeIsActive()) {
            ...
            if(frame < 0){
                if(pipeline.position.equals("right")){
                    teleOpVoltage = Double.parseDouble(strReadFromFile("voltage" + fileStart +"brr.txt"));
                    correctLooptime = teleOpVoltage = Double.parseDouble(strReadFromFile("looptime" + fileStart +"brr.txt"));
                    byteArrList = brr;
                }
                else if(pipeline.position.equals("middle")){
                    teleOpVoltage = Double.parseDouble(strReadFromFile("voltage" + fileStart + "brm.txt"));
                    correctLooptime = teleOpVoltage = Double.parseDouble(strReadFromFile("looptime" + fileStart +"brm.txt"));

                    byteArrList = brm;

                }
                else if (pipeline.position.equals("left")){
                    teleOpVoltage = Double.parseDouble(strReadFromFile("voltage" + fileStart +"brl.txt" ));
                    correctLooptime = teleOpVoltage = Double.parseDouble(strReadFromFile("looptime" + fileStart +"brl.txt"));
                    byteArrList =brl;

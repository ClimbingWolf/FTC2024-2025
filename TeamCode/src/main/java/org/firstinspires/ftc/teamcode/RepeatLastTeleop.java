...
@Autonomous(name = "BlueAutoBoard")
@Config
public class RepeatLastTeleop extends LinearOpMode {
    ...
    public double servoOffsetTop = 240;//pidConstantsAndFunctions.servoOffsetTop;
    ...
    //constants for math
    OpenCvWebcam webcam;
    CameraDetectPipelineBlue pipeline;
    String position = "";
    ...
    public static double voltageExtraMultiplier = 1;
    ...
    public void runOpMode() {
        ...
        pipeline = new CameraDetectPipelineBlue();
        ...
        claw.setPosition(posB);
        blr = readFromFile(fileStart + "blr.txt");
        blm = readFromFile(fileStart +"blm.txt");
        bll = readFromFile(fileStart +"bll.txt");
        ...
        while (opModeIsActive()) {
            ...
            if(frame < 0){
                if(pipeline.position.equals("right")){
                    teleOpVoltage = Double.parseDouble(strReadFromFile("voltage" + fileStart +"blr.txt"));
                    correctLooptime = teleOpVoltage = Double.parseDouble(strReadFromFile("looptime" + fileStart +"blr.txt"));
                    byteArrList = blr;
                }
                else if(pipeline.position.equals("middle")){
                    teleOpVoltage = Double.parseDouble(strReadFromFile("voltage" + fileStart + "blm.txt"));
                    correctLooptime = teleOpVoltage = Double.parseDouble(strReadFromFile("looptime" + fileStart +"blm.txt"));

                    byteArrList = blm;

                }
                else if (pipeline.position.equals("left")){
                    teleOpVoltage = Double.parseDouble(strReadFromFile("voltage" + fileStart +"bll.txt" ));
                    correctLooptime = teleOpVoltage = Double.parseDouble(strReadFromFile("looptime" + fileStart +"bll.txt"));
                    byteArrList =bll;
